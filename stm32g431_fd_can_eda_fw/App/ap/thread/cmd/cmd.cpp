...
bool cmdThreadInit(void)
{
  ...
  cmd_uart::InitDriver(&cmd_drvier[0], HW_UART_CH_USB, 1000000);
...
bool cmdThreadUpdate(void)
{
  ...
  for (int i=0; i<CMD_DRIVER_MAX_CH; i++)
  {
    if (cmd[i].is_init == true)
    {
      ...
      {
        bool ret = false;
        ret |=  cmd_boot::Process(&cmd[i]);//cmdBootProcess(&cmd[i]);
      ...
      cmd_boot::Update(&cmd[i]);//cmdBootUpdate(&cmd[i]);
    }
  }
